...
#if SDIO_DMA_ENABLE
static sd_data_transfer_mode_type device_mode = SD_TRANSFER_DMA_MODE; /* working mode */
#else
...
sd_error_status_type sdio_data_command_flag_service(sdio_command_struct_type *sdio_cmd_init_t, uint32_t timeout);
...
sd_error_status_type sd_init(void)
{
  ...
  if((status = sd_card_init()) != SD_OK)
		return status;

	if((status = sd_card_info_get(&sd_card_info)) != SD_OK)
		return status;
  ...
  if((status == SD_OK) || (card_type == SDIO_MULTIMEDIA_CARD))
  {
	...
	sdio_bus_width_config(SDIOx, SDIO_BUS_WIDTH_D4);
...
/**
  * @brief  read data from or write data flag service.
...
sd_error_status_type sdio_data_command_flag_service(sdio_command_struct_type *sdio_cmd_init_t, uint32_t timeout)
{
  ...
  while(sdio_flag_get(SDIOx, SDIO_DTCMPL_FLAG) != SET && timeout)
  {
    timeout --;
  }
                                            ...
                                            sdio_data_struct_type* sdio_data_init_t, uint32_t *buf)
{
  ...
  if(device_mode == SD_TRANSFER_POLLING_MODE)
  {
    ...
    return sdio_data_command_flag_service(sdio_cmd_init_t, timeout);
  }
#if SDIO_DMA_ENABLE
...
{
	dma_channel_enable(SDIO_DMA_CHANNEL, FALSE);
	SDIO_DMA_CHANNEL->dtcnt = buf_size / 4;
	SDIO_DMA_CHANNEL->paddr = (uint32_t)&SDIOx->buf;
	SDIO_DMA_CHANNEL->maddr = (uint32_t)mbuf;
	SDIO_DMA_CHANNEL->ctrl &= 0xbfef;
	SDIO_DMA_CHANNEL->ctrl |= dir;
	dma_channel_enable(SDIO_DMA_CHANNEL, TRUE);
}
#endif
